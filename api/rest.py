...
class PatchSerializer(BaseMessageSerializer):
    class Meta:
        model = Message
        fields = BaseMessageSerializer.Meta.fields + ('last_comment_date', 'patch_num')

class SeriesSerializer(BaseMessageSerializer):
    class Meta:
        model = Message
        fields = ('resource_uri',) + BaseMessageSerializer.Meta.fields + (
             'stripped_subject', 'last_reply_date', 'is_complete', 'is_merged', 'message', 'results')
...
class SeriesViewSet(BaseMessageViewSet):
    ...
    queryset = Message.objects.filter(is_series_head=True)
